...
- 本 p.112 のコード `4-2-2` と同様の結果を得るため、最初に学習会独自の `record_auto_stop` 関数（この章向け）を以下の手順で作ります
...
- この関数と、前項で作った関数を使い「マイク録音→音声認識→丁寧語変換」を短いコードで行います. 新しいセルに以下のコードを書きます
...
### 4-4. 変換パターンを増やそう（p.121〜）

- 前項でやった丁寧語変換をちゃんと実装するには、語尾だけの「だ」を「です」に変換したり様々な工夫が要ります. それを行うのが本の `4-4` で、「正規表現」という文法や、変換しない語を「ダミー文字列」にして後で戻す等、込み入った処理をします. それはひとまず割愛し（次章以降の、カメラを使う画像処理や物体検知を早めに体験してもらうため）、希望があれば後で実施します

<br>

### 4-5. 繰り返し変換できるようにしよう（p.134〜）

- Chapter 4の最後のセクション `4-5` で登場する「繰り返し処理」を行います
...
- 実行すると 🎤 が表示され、無音が2秒続くと「録音終了」になって再び 🎤 が出るという状態が続きます

- 🎤 表示の間に何か話すと音声認識が始まり、認識結果と丁寧語変換結果が表示されます
