...
- 丁寧語変換をちゃんと実装するには、語尾だけの「だ」を「です」に変換したり様々な工夫が要ります. それを行うのが本の次節 `4-4` で、「正規表現」という文法や、変換しない語を「ダミー文字列」にして後で戻す等、込み入った処理をします. それはひとまず割愛し（次章以降の、カメラを使う画像処理や物体検知を早めに体験してもらうため）、希望があれば後で実施します
